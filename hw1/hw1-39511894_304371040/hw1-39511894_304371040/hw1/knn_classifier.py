...
class KNNClassifier(object):
    ...
    def train(self, dl_train: DataLoader):
        ...
        first = True
        for idx, (images, labels) in enumerate(dl_train):
            if first is True:
                x_train = images
                y_train = labels
                first = False
        ...
        self.n_classes = torch.unique(y_train).size(0)
        return self
    ...
    def predict(self, x_test: Tensor):
        ...
        for i in range(n_test):
            indices = dist_matrix[:,i].argsort().narrow(dim=0,start=0,length=self.k)
            classes = torch.index_select(self.y_train, 0, indices).to(dtype=torch.float32)
            y_pred[i] = int(np.bincount(classes).argmax())
...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    aa = torch.sum(x1 ** 2, dim=1)
    bb = torch.sum(x2 ** 2, dim=1)
    ab = -2 * torch.mm(x1, x2.transpose(0, 1))
    dists = torch.sqrt((aa.view(aa.size(0), 1) + ab) + bb)
...
def accuracy(y: Tensor, y_pred: Tensor):
    ...
    accuracy = float(torch.sum(y == y_pred)) / float(y.size(0))
    return accuracy
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    for i, k in enumerate(k_choices):
        ...
        for j in range(num_folds):
            dl_train, dl_valid = hw1dataloaders.create_train_validation_loaders(ds_train, 1 / float(num_folds))
            x_valid, y_valid = dataloader_utils.flatten(dl_valid)
            model.train(dl_train)
            y_pred = model.predict(x_valid)
            current_accuracy.append(accuracy(y_valid, y_pred))
            print('fold = ' + str(j))
